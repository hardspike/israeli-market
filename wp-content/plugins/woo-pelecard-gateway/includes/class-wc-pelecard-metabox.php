...
class WC_Pelecard_Metabox {
	...
	public function admin_scripts() {
		...
		if ( 'shop_order' === $screen_id ) {
			...
			wp_enqueue_script(
				'wc-pelecard-gateway-metabox', 
				plugins_url( '/assets/js/metabox' . ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min' ) . '.js', WC_PELECARD_PLUGIN_FILE ), 
	...
	public function add_meta_boxes() {
		...
		if ( 'shop_order' === $screen_id && ( self::$transactions = get_post_meta( $post->ID, '_transaction_data' ) ) ) {
			add_meta_box( 'pelecard-transaction-data', __( 'Pelecard Transaction Data', 'woo-pelecard-gateway' ), array( $this, 'transaction_data' ), 'shop_order', 'advanced', 'default' );
		}
	}
	
	/**
	 * Pelecard transaction data
	 *
	 * @param  object $post
	 * @return void
	 */
